...
4. Add the proper entries for wifi_ssid, wifi_password, ota_password, and *api_password to your secrets.yaml file in the ESPhome folder.  I have included an example if you dont already have these, but it must be customized with your desired passwords.  The ESPhome web interface now has a button for this in the upper right corner, but you may need to create the file first for this button to appear.  *Note, ESPhome has recently changed to using API encryption key instead of a password, so you may see a deprecated notice until you move to that setup.  I will update the project her soon for this change.
